"""Router network for choosing codelet, lambda, and denoiser given x_k."""
...
def extract_scalar_features(x: torch.Tensor, x_prev: torch.Tensor, A: torch.Tensor, y: torch.Tensor, k: int, K: int) -> torch.Tensor:
    """Cheap scalar features summarizing progress and gradients."""
    B = x.shape[0]
    x_flat = x.view(B, -1)
...
class CodeletDenoiserRouter(nn.Module):
    def __init__(self, state_dim: int, hidden: int = 64):
        ...
        self.head_codelet = nn.Linear(hidden, 5)
        self.head_lambda = nn.Linear(hidden, 5)
        self.head_deno = nn.Linear(hidden, 5)
